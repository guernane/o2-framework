...
Together, these two repositories constitute the complete, versioned state
of the project — `analysis/` lives inside this repository as a module, not
as a separate one. Ephemeral build artefacts (compiled binaries, container
images, downloaded data, run outputs under `analysis/*/output/` and
`analysis/*/bookkeeping/`) are deliberately excluded from version control
and are regenerated locally as described in [Section 4](#4-provisioning-a-new-machine).
...
o2 build --rebuild-tasks       # sync user tasks (analysis/) and rebuild
...
### 1.4 `o2 status`
Reports the state of configured analysis, HPC job queues, and the validity
...
### 1.6 `o2 analysis` *(local workstation only)*
Manages the analysis registry (`analysis/analysis.json`): enabling or
disabling individual analysis and their constituent tasks.
```bash
o2 analysis --list
o2 analysis --enable <workflow>
o2 analysis --disable <workflow>
```
...
Commits and pushes outstanding local changes in `o2-framework` to its GitHub repository. `O2Physics` is
...
o2 export --repos "o2-framework"                   # exclude O2Physics entirely
o2 export --o2physics-paths "PWGJE,PWGCF,Common"   # custom top-level subset
o2 export --o2physics-paths "ALL"                  # full O2Physics source
o2 export --analysis-paths "test"                  # a single workflow only
o2 export --keep-artifacts                         # retain output/ and bookkeeping/
o2 export ~/Desktop/snapshot.tar.gz                # custom output path
```

### 1.10 `o2 list` *(local or remote, or both)*
Reports what currently exists in each of three independent categories,
without deleting anything: downloaded AO2D data, run output directories,
and O2Physics dev-build versions.
```bash
o2 list --both                    # all categories, local and cluster
o2 list outputs --local           # a single category
...
For `data` and `outputs`, an item may be removed by name (as reported by
`o2 list`), or in bulk via `--all` or `--keep N` (retaining the N most
...
For `builds`, the command delegates entirely to aliBuild's own native
`aliBuild clean` (run inside the container), rather than attempting a
hand-rolled removal. This choice was verified directly against aliBuild's
source (`alibuild_helpers/clean.py`): aliBuild's `decideClean()` removes
`$workDir/TMP`, `$workDir/INSTALLROOT`, any `$workDir/BUILD/*` not
referenced by a `BUILD/*-latest*` symlink, and any installed package
version not referenced by that package's own `latest*` symlink — which is
both more complete (it also reclaims `sw/BUILD/` and `sw/INSTALLROOT/`,
which a naive cleanup targeting only `dev-local*` directories would miss)
and consistent with the tool the ALICE build system itself relies on. The
`--aggressive` flag maps to aliBuild's `--aggressive-cleanup`, which
additionally removes the tarball store and source checkouts; this frees
more space but can invalidate cached builds that reference removed
tarballs (see [alisw/alibuild#412](https://github.com/alisw/alibuild/issues/412)).
No `--all`/`--keep`/item name applies to this category — aliBuild's clean
is holistic, not selective by version:
```bash
...
A note on cluster filesystem behaviour: a process left in uninterruptible
sleep (`D` state) on a networked filesystem such as `/bettik` can cause
subsequent operations on the same path to hang indefinitely, independently
of this command or of aliBuild itself. If `o2 clean builds --remote`
appears stuck for an unusually long time, first verify whether the
underlying path is responsive at all (e.g. `ssh <host> "timeout 10 ls
<path>"`, run directly on the cluster rather than piped through a fresh
non-interactive SSH session) before assuming a fault in this command.
...
├── lib/                                                       ← this repository
├── analysis/             ← this repository (module, see Section 2)
...
- Synchronisation of `analysis/` to the HPC cluster currently requires an
  ...
  `analysis/` to reside on the cluster at all) is planned but not yet
  implemented.
